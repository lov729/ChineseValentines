...
const App = () => {
  ...
  const pigeonHeadRef = useRef(null);
  const pigeonBeakRef = useRef(null);
  ...
  useEffect(() => {
    ...
    // 添加头部朝向信封的动画
    const pigeonHead = pigeonHeadRef.current;
    const pigeonBeak = pigeonBeakRef.current;
    
    const headAnimationTimer = setTimeout(() => {
      if (pigeonHead && pigeonBeak) {
        pigeonHead.style.animation = 'lookAtEnvelope 3s infinite ease-in-out';
        pigeonBeak.style.animation = 'lookAtEnvelope 3s infinite ease-in-out';
      }
    }, 6000);

    // 信封交互
    const handleHeartSealClick = () => {
      if (letterRef.current && envelopeRef.current && envelopeFlapRef.current && pigeonHeadRef.current && pigeonBeakRef.current) {
        letterRef.current.style.display = 'block';
        envelopeRef.current.style.opacity = '0';
        envelopeFlapRef.current.style.transform = 'rotateX(180deg)';
        
        // 停止头部动画
        pigeonHeadRef.current.style.animation = 'none';
        pigeonBeakRef.current.style.animation = 'none';
      }
    };
    ...
    const handleCloseBtnClick = () => {
      if (letterRef.current && envelopeRef.current && envelopeFlapRef.current && pigeonHeadRef.current && pigeonBeakRef.current) {
        ...
        envelopeFlapRef.current.style.transform = 'rotateX(0deg)';
        
        // 重新开始头部动画
        setTimeout(() => {
          pigeonHeadRef.current.style.animation = 'lookAtEnvelope 3s infinite ease-in-out';
          pigeonBeakRef.current.style.animation = 'lookAtEnvelope 3s infinite ease-in-out';
        }, 500);
      }
    };

    const heartSeal = heartSealRef.current;
    if (heartSeal) {
      heartSeal.addEventListener('click', handleHeartSealClick);
    }
    ...
    // 清理函数
    return () => {
      clearTimeout(headAnimationTimer);
      if (heartSeal) {
        heartSeal.removeEventListener('click', handleHeartSealClick);
      }
      if (closeBtn) {
      ...
      <div className="scene">
        ...
        <div className="pigeon-container">
          <div className="envelope" ref={envelopeRef}>
            <div className="envelope-flap" ref={envelopeFlapRef}></div>
            <div className="heart-seal" ref={heartSealRef}></div>
          </div>
          
          <div className="pigeon">
            <div className="pigeon-body"></div>
            <div className="pigeon-wing"></div>
            <div className="pigeon-head" ref={pigeonHeadRef}></div>
            <div className="pigeon-beak" ref={pigeonBeakRef}></div>
            <div className="pigeon-eye"></div>
          </div>
        </div>
